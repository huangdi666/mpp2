...
int    HI_OSD_Parse_OsdTitle(HH_OSD_LOGO *pOsdLogo)
{
	...
	while (pOsdLogo->szOsdTitle[index] != '\0' && pOsdLogo->nOsdLine <= HH_MAX_TITLE_LINE)
	{
		...
		{
			pOsdLogo->nOsdTitleLen[pOsdLogo->nOsdLine-1] = index - pOsdLogo->nOsdTitleFlag[pOsdLogo->nOsdLine-1];
			pOsdLogo->nOsdLine    += 1;
		...
		else if (pOsdLogo->szOsdTitle[index] > 0x7F)	//处理中文字符
		{
...
//	stRgnAttr.unAttr.stOverlay.u32CanvasNum = 6; //hsx
	...
	printf( "HI_Create_Osd_Reg handle  = %d (%d, %d) with = %d height = %d (group : %d)(unknow : %d)\n",
		pOsdLogo->regHdl,
...
int HI_OSD_Build(HH_OSD_LOGO *pOsdLogo)
{
	...
	HI_OSD_Parse_OsdTitle(pOsdLogo);//对字符分行解析

	HI_OSD_Get_BmpSize(pOsdLogo);//这里设置pOsdLogo->nOsdFontW 为32(原本是0)
	char *p = "测"; 
	printf("0x%x,0x%x\n11111111111111111111111",*p,*(p+1));
					  ...
					  HI_U16 fColor, HI_U16 bColor, HI_U16 edgeColor)
{
	...
	while (*(pstr+i) != '\0')
	{
		if (*(pstr+i) >= 128)
		{
			qu = *(pstr+i) - 160;//对应的中文字符减去160，是对于的区。由编译器翻译成区位码
			i++;
			if (*(pstr+i) >= 128)
			{
				hzCount++;
