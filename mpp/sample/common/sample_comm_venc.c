...
int SAMPLE_COMM_VENC_MP4(VENC_STREAM_S *stStream)
{
	...
	if(recording && stStream->u32Seq > 100){
	...
	if((recording && stStream->u32Seq > 60000)){
		recording = 0x00;
...
* funciton : get stream from each channels and save them
******************************************************************************/
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc_bak(HI_VOID *p)
{
	int nRecordFlag = 0x00;
	int recording = 0x1;
	int j = 0;
	int spsflag = 0;
	int ppsflag = 0;
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
					...
					if(recording && stStream.u32Seq > 300){
						if(stStream.u32PackCount >= 3){
							nRecordFlag = 1;
						}
					...
					if((recording && stStream.u32Seq > 2500)){
						recording = 0x00;
						printf("Close mp4 file &&&&&&&&&&&&&&&&&&&&&&&&&&\n");						
						MP4Close(hMP4File, 0);
						hMP4File = NULL;
