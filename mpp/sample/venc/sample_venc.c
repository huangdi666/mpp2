...
int SAMPLE_RGN_LoadBMPCanvas_Logo(char* filename,int resolution, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
	...
	HI_U8* pOrigBMPBuf;	//新开的一个buffer，指向来源的bmp数据
	HI_U8* pRGBBuf;		//这个buffer用来显示，相当于缓存的framebuffer
	HI_U32 stride;
	HI_U8 r, g, b;
	HI_U8* pStart;		//这两个用来做数据处理，指向源buffer
	HI_U16* pDst;		//这两个用来做数据处理，这个指向显示buffer
...
int SAMPLE_RGN_LoadPNGCanvas_Logo(char* filename,int resolution, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
	FILE* pFile;
	HI_U16  i, j;
	...
	if (GetPngInfo(filename) < 0)
	{
		return -1;
	}
	
	printf("((((((((((((((OSD_Loadpng OK!!!!!!!!!!!!!!!   %s\n",filename);
    ...
    //draw_image(0, 0, 1024, 600, pdata, MODE_JPG);
	//printf("%d ",*(pOrigBMPBuf + i * stride + j);
//	for (i = 0; i < h; i++)  // Bpp:3,enFmt:4
//	{
//		for (j = 0; j < w * Bpp; j++)
//			printf("0x%x ",*(pOrigBMPBuf + i * stride + j));
//		printf("\n i = %d\n ",i);
//	}
	for (i = 0; i < h; i++)  // Bpp:3,enFmt:4
	{
		...
		for (j = 0; j < w; j++)
		{
			if (Bpp == 3) /*.....*/
			{
				...
				case OSD_COLOR_FMT_RGB4444:
					...
					// h:48,w:32,stride:96  Canvas_h:48,Can_w:704,Can_Str:1408
					//pStart = pOrigBMPBuf + ((h - 1) - i) * stride + j * Bpp;
					pStart = pOrigBMPBuf + i * stride + j * Bpp;
...
int SAMPLE_RGN_LoadJPGCanvas_Logo_hd(char* filename,int height,int width,int stride1,char *pbuffer, OSD_COLOR_FMT_E enFmt)
{
	if (NULL == filename)
	{
		printf("OSD_LoadBMP: filename=NULL\n");
	...
	HI_U8* pOrigBMPBuf;	//新开的一个buffer，指向来源的bmp数据
	pOrigBMPBuf = (HI_U8*)malloc(cinfo.output_height * stride);
	if (NULL == pOrigBMPBuf)
	{
		printf("not enough memory to malloc!\n");
		fclose(pFile);
	...
	{	 
		...
		// 来给循环的下一次解码一行来使用
		memcpy(pOrigBMPBuf + (cinfo.output_scanline-1) * stride, buffer, stride);
	}
	HI_U32  w, h;
	HI_U16 Bpp;
	...
	for (i = 0; i < h; i++)  
		for (j = 0; j < w; j++)
			...
			{
				pStart = pOrigBMPBuf + i * stride + j * Bpp;
				pDst = (HI_U16*)(pRGBBuf + i * stride1 + j * 2);
				r = *(pStart+2);
				g = *(pStart + 1);
				b = *(pStart);
				*pDst = OSD_MAKECOLOR_U16(r, g, b, s_OSDCompInfo[4]);
			}
...
int SAMPLE_RGN_LoadJPGCanvas_Logo(char* filename,int resolution, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
	if (NULL == filename)
	{
		printf("OSD_LoadBMP: filename=NULL\n");
...
int SAMPLE_RGN_LoadCanvasEx(int Type, OSD_LOGO_T* pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
	...
	if(Type == VENC_RECORD_LOGO_OSD_HANDLE){		
		...
		//sprintf(filename,"%s","res/logo_1080p.png");
		//nRet= SAMPLE_RGN_LoadPNGCanvas_Logo(filename,0, pVideoLogo, enFmt);
		sprintf(filename,"%s","res/logo_1080p.jpg");
		nRet= SAMPLE_RGN_LoadJPGCanvas_Logo(filename,0, pVideoLogo, enFmt);
	}else{
...
HI_S32 SAMPLE_RGN_CreateSurfaceByCanvas_hd(int Type, BITMAP_S* pstBitmap, HI_U32 u32Stride)
{
	char filename[32] = {0};
	sprintf(filename,"%s","res/logo_1080p.jpg");
	if(SAMPLE_RGN_LoadJPGCanvas_Logo_hd(filename,pstBitmap->u32Height,pstBitmap->u32Width,u32Stride,pstBitmap->pData, pstBitmap->enPixelFormat)<0)
	{
...
HI_S32 SAMPLE_RGN_UpdateCanvas(int Type, BITMAP_S* pstBitmap, HI_BOOL bFil, HI_U32 u16FilColor, HI_U32 u32Stride, PIXEL_FORMAT_E enPixelFmt)
{
...
//这里进行画布buffer的填充，注意给进来的bmp或者jpg可能是3字节位深，但这里的是2字节位深。需要转换
	SAMPLE_RGN_CreateSurfaceByCanvas_hd(Type, pstBitmap, u32Stride);

	//透明度处理，如果数据是0or全f，那么透明处理
	int i = 0, j = 0;
...
/**
 * 
 *  首先创建一个区域，类型是overlay；然后把该区域叠加到通道中
 * 
 * 
 * 
 */
HI_S32 SAMPLE_RGN_CreateOverlayForVenc(HI_U32 u32ChnId, RGN_HANDLE Handle, HI_U32 u32X, HI_U32 u32Y, HI_U32 u32Width, HI_U32 u32Height)
{
	...
	******************************************/
	...
	stRgnAttr.enType = OVERLAY_RGN;	//设置区域类型
	...
	stRgnAttr.unAttr.stOverlay.u32BgColor       = 0/*0xffffffff*//*0x000003e0*/;
	...
	******************************************/
	...
	stChnAttr.unChnAttr.stOverlayChn.u32BgAlpha   = 50/*128*/; //Bg: transparent
	...
	/******************************************
	 step  3: 设置好画布信息，填充buffer
	******************************************/

	int bpp = 2;
	BITMAP_S stBitmap;
	stBitmap.pData   = (char *)malloc(u32Width*u32Height*bpp);
	stBitmap.u32Width  = u32Width;
	stBitmap.u32Height = u32Height;
	s32Ret = SAMPLE_RGN_UpdateCanvas(Handle, &stBitmap, HI_TRUE , 0x0000, u32Width*bpp,8);
	...
	******************************************/
	...
	if (HI_SUCCESS != s32Ret)
	{
		printf("HI_MPI_RGN_SetBitMap! s32Ret: 0x%x.\n", s32Ret);
		return s32Ret;
	}
...
/*
	这里是填充画布
	该函数主要针对把stBitmap.pData这个buffer填充完整
*/
	BITMAP_S stBitmap;
	...
	s32Ret = SAMPLE_RGN_UpdateCanvas(Type, &stBitmap, HI_TRUE , 0x0000, stCanvasInfo.u32Stride,stRgnAttrSet.unAttr.stOverlayEx.enPixelFmt);
	if (HI_SUCCESS != s32Ret)
	{
		printf("SAMPLE_RGN_UpdateCanvas failed! s32Ret: 0x%x.\n", s32Ret);
		return s32Ret;
	...
	s32Ret = HI_MPI_RGN_UpdateCanvas(Handle);//刷新画布
	if (HI_SUCCESS != s32Ret)
	{
